        ...
        if result.returncode == 0:
            for line in result.stdout.splitlines():
                ...
                if len(parts) >= 6 and parts[-1] == label:
                    ...
                    partition_path = f"{device_path}{partition_num}"
                    
                    # Wait for device node to appear
                    if os.path.exists(partition_path):
                        logger.info(f"Found partition {label} at {partition_path}")
                        return partition_path
                    else:
                        logger.warning(f"Partition {label} found in GPT but device node {partition_path} does not exist yet")
...
def write_files_to_partition(
    ...
    try:
        size_result = subprocess.run(
            ["blockdev", "--getsize64", partition_path],
            capture_output=True,
            text=True,
            check=True
        )
        partition_size_bytes = int(size_result.stdout.strip())
        logger.info(f"Partition size: {partition_size_bytes} bytes "
                   f"({partition_size_bytes // (1024*1024)} MB)")
    except (subprocess.CalledProcessError, ValueError) as e:
        raise USBWriteError(f"Failed to get partition size: {e}")
    ...
    try:
        # Create empty filesystem image of exact partition size
        logger.info(f"Creating temporary FAT filesystem image")
        subprocess.run(
            ["truncate", "-s", str(partition_size_bytes), fs_image_path],
            check=True,
            capture_output=True
        )
