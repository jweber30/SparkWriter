...
def extract_github_username_from_url(url: str) -> Optional[str]:
    ...
    if hostname == "github.com":
        # Supports /owner/repo/blob/<ref>/<path> and /owner/repo/raw/<ref>/<path>
        if len(path_parts) >= 2:
            return path_parts[0].lower()
        return None
...
def normalize_github_manifest_url(url: str) -> str:
    """Normalize GitHub manifest URLs into raw-compatible form when possible."""
    parsed = urllib.parse.urlparse(url)
    hostname = (parsed.hostname or "").lower()
    path_parts = [p for p in parsed.path.split("/") if p]

    if hostname == "github.com" and len(path_parts) >= 5 and path_parts[2] in {"blob", "raw"}:
        owner = path_parts[0]
        repo = path_parts[1]
        ref = path_parts[3]
...
def build_manifest_download_request(url: str, token: Optional[str] = None) -> urllib.request.Request:
    """Build a request suitable for manifest download.

    For authenticated downloads against GitHub private repositories, map raw URLs
    to the GitHub contents API and request raw content with an access token.
    """
    normalized = normalize_github_manifest_url(url)
    ...
    if token and hostname == "raw.githubusercontent.com" and len(path_parts) >= 4:
        owner = path_parts[0]
        repo = path_parts[1]
        ref = path_parts[2]
    ...
    return urllib.request.Request(normalized, headers=headers, method="GET")
